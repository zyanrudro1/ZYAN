...
login_gate() {
    ...
    # Approved trial usernames; all use the same password.
    local -a valid_users=("ZYAN" "ZYAN87" "ZYAN10" "ZYAN80" "ZYAN99")
    local shared_password="12"

    # Keep device bindings local to this Linux installation.
    binding_dir="${HOME}/.zyancam"
    binding_file="${binding_dir}/device_bindings"
    mkdir -p "$binding_dir"
    chmod 700 "$binding_dir"
    touch "$binding_file"
    chmod 600 "$binding_file"

    if [[ -r /etc/machine-id ]] && [[ -n "$(cat /etc/machine-id 2>/dev/null)" ]]; then
        machine_id=$(cat /etc/machine-id 2>/dev/null)
    elif [[ -r /sys/class/dmi/id/product_uuid ]] && [[ -n "$(cat /sys/class/dmi/id/product_uuid 2>/dev/null)" ]]; then
        machine_id=$(cat /sys/class/dmi/id/product_uuid 2>/dev/null)
    else
        machine_id=$(hostname 2>/dev/null)
    fi

    while true; do
        clear
        printf "\e[1;92m             ZYANCAM\e[0m\n"
        printf "\e[1;77m        Dev-ZYAN-RUDRO\e[0m\n\n"
        read -r -p $'Username: ' username

        for key in "${valid_users[@]}"; do
        ...
        if [[ "$password" != "$shared_password" ]]; then
            printf "\n\e[1;31m[!] Invalid password.\e[0m\n"
            sleep 1.2
            continue
        fi

        # One username is allowed to bind to one device ID.
        existing_device=$(awk -F'|' -v u="$username" '$1 == u {print $2; exit}' "$binding_file")
        if [[ -n "$existing_device" && "$existing_device" != "$machine_id" ]]; then
            printf "\n\e[1;31m[!] This username is already bound to another device.\e[0m\n"
            sleep 1.5
            continue
        fi

        if [[ -z "$existing_device" ]]; then
            printf '%s\n' "$username|$machine_id" >> "$binding_file"
        fi

        printf "\n\e[1;92m[+] Login successful.\e[0m\n"
        sleep 0.8
        return 0
    done
}
...
printf "\e[1;92m  ███         ███    ███ ███    ███ ███      \e[0m\n"
printf "\e[1;92m  ███         ▀██████▀   ▀██████▀  ███      \e[0m\n"
printf "\e[1;93m             ZYANCAM\e[0m\n"
printf " \e[1;77m Dev-ZYAN-RUDRO | youtube.com/2040CHEATS \e[0m \n"
...
printf "\e[1;92m[\e[0m*\e[1;92m] \e[0m\e[1;93m ZYAN CAM tunnel service might be down\n"
...
printf "\n-----Choose a template----\n"    
printf "\n\e[1;92m[\e[0m\e[1;77m01\e[0m\e[1;92m]\e[0m\e[1;93m CAM Wishing\e[0m\n"
...
if [[ $option_tem -eq 1 ]]; then
read -p $'\n\e[1;92m[\e[0m\e[1;77m+\e[0m\e[1;92m] Enter CAM name: \e[0m' fest_name
